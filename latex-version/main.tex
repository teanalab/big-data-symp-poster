...
\begin{block}{infNDCG retrieval metric by varying the weight of one of the features}
  \begin{figure}[!htb]
  \centering
  \includegraphics[width=\textwidth]{../intgr-cds-acm-version/images/htbigramprf.eps}
  \end{figure}
...
\begin{alertblock}{The first three iterations\ldots}
  \begin{figure}[!htb]
  \centering
  \subfigure[1$^{\rm st}$ Iteration]{\label{fig:smooth:a}\includegraphics[width=0.3\textwidth]{../intgr-cds-acm-version/images/optIter1Htbigramprf.eps}}
  \subfigure[2$^{\rm nd}$ Iteration]{\label{fig:smooth:b}\includegraphics[width=0.3\textwidth]{../intgr-cds-acm-version/images/optIter2Htbigramprf.eps}}
  \subfigure[3$^{\rm rd}$ Iteration]{\label{fig:smooth:c}\includegraphics[width=0.3\textwidth]{../intgr-cds-acm-version/images/optIter3Htbigramprf.eps}}
  \end{figure}
\end{alertblock}
...
\scriptsize
\begin{algorithmic}[1]
    \STATE{Identify explicit and latent concepts}
    \STATE{Randomly initialize the feature weights vector ($\mathbf{w}_\phi$)}
    \FOR{$j=1:j_{\max}$}
          \STATE{Randomly shuffle $\mathbf{w}_\phi$}
        \FOR{$n=1:N$}
            \FOR{each sampling policy}
                \STATE{Sample $E^{n,j}(w_{\phi}^{n})$}
                \STATE{Obtain $\tilde E^{n,j}(w_{\phi,m}^{n})$}
                \STATE{Obtain the optimum point  $\hat w_{\phi}^{n}$}
                   \STATE{Update $n$-th element of $\mathbf{w}_\phi$ by $\hat w_{\phi}^{n}$}
            \ENDFOR{}
        \ENDFOR{}
        \IF{Convergence}
               \STATE{Break}
